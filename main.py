...
def main():
    try:
        ...
        permission = input("Would You Like To Start?\nY/N\n\n >>> ")

        try:
            with open('.data.yaml', 'r') as Reader:
                Process = yaml.safe_load(Reader)
                Target = Process["Colected_Data"]["Target"]
                User_Agent_List = Process["Fake_Credentials"]["User_Agent_List"]
                Proxy_List = Process["Fake_Credentials"]["Proxy_List"]
                Site_Protocol = Process["Script_Override"]["Site_Protocol"]
                Site_Timeout = Process["Script_Override"]["Site_Timeout"]
                Threading_Value = Process["Script_Override"]["Threading_Value"]
        except KeyError:
            print('Some ".data.yaml" File Data Is Unreadable!!!')
            print('Resetting Your Script... (Friendly Note: Only ".data.yaml" Will Be Removed)')
            if os.name == "posix":
                os.system(f'rm -rf {os.path.dirname(os.path.abspath(__file__))}/.data.yaml')
            else:
                os.system(f'del {os.path.dirname(os.path.abspath(__file__))}\\.data.yaml /q')
        ...
        def Check_Internet_Connection():
            print("Checking For User's Internet Connection...")
            try:
                requests.get(url="https://camo.githubusercontent.com/12df27140682753703f98aa63ea1afe69472864e061805883667dc107d86efcb/68747470733a2f2f6b6f6d617265762e636f6d2f67687076632f3f757365726e616d653d52656446757272466f78", proxies={f'{Site_Protocol}': f'{Site_Protocol}://{random.choice(Proxy_List)}'}, timeout=Site_Timeout)
                pass
            except (requests.RequestException, requests.ConnectionError, requests.ConnectTimeout):
                print("You Are Offline... Please Check Your Internet Before Using This Script.")
                exit()
        ...
        if permission == "y":
            ...
            clear()
            Type = input('What Type Of Method Do You Want To Use?\n\nChoose 1 For "One Request At A Time (Safe)",\nChoose 2 For "Multi-Threading (Unsafe)",\nChoose 3 For "Script Reset (Includes: Github Tag/Shield Link And Other Modifications to ".data.yaml"),\nAnd Choose 0 To Exit)".\n\n >>> ')

            def attack_1():
                def request():
                    while True:
                        try:
                            manager = requests.get(url=Target, headers=Header_Data, proxies={f'{Site_Protocol}': f'{Site_Protocol}://{random.choice(Proxy_List)}'}, timeout=Site_Timeout)
                            print(f'\nRequest Sent ✔ || Website Status Code: "{manager.status_code}"')
                            attack_1()
                        except requests.ReadTimeout:
                            print(f'\nRequest Blocked ❌ || Reason: ReadTimeout Error')
                            attack_1()
                            continue
                        except (requests.RequestException, requests.ConnectionError, requests.ConnectTimeout):
                            print(f'\nRequest Not Sent ❌ || Reason: Offline')
                            attack_1()
                            continue
                request()
            ...
            def attack_2():
                ...
                def threads():
                    ...
                    for i in range(Threading_Value):
                        threads[i].join()
                print("Pre-Loading Threads.\n\n")
                threads()

            if Type == "1":
                clear()
                print("Starting The Request Spam By One At A Time.\n\n")
                attack_1()
            elif Type == "2":
                clear()
                print("Starting The Request Spam By Multi-Threading.")
                attack_2()
            elif Type == "3":
                clear()
                print('Resetting Your Script... (Friendly Note: Only ".data.yaml" Will Be Removed)')
                ...
                print(f'{Type} Not Found, Please Try Again.')
                exit()
...
if __name__ == "__main__":
    ...
    if os.path.isfile(".data.yaml"):
        print("You Are Currently Using The Encrypted Version Of Github View Spammer")
        time.sleep(1)
        main()
    else:
        try:
            with open(".data.yaml", "w") as Writer:
                Tag_Link = input("\nEnter Your Tag Link (Case Sensitive)? ")
                Writer.write(f"""# Friendly Reminder: Only Modify What You Understand To Avoid Errors

Colected_Data:   # This is what the script collected to you:
  Target: {Tag_Link}   # Your Personal Link

Fake_Credentials:   # To avoid getting Blocked
...
Script_Override:
  Site_Protocol: http  # Choices: http or https (Note: Some proxy doesn't support https)
  Site_Timeout: 5   # Default "5" (In seconds)
  Threading_Value: 15  # Do Not Change!!! "This is the safest value!!!" (Default: 15) """)
                Writer.close()
            main()
        except (KeyboardInterrupt, EOFError):
            print("\n\nExiting...")
